...
#include <iostream>
#include "4081_and.hpp"

nts::And4081::And4081() noexcept
{
...
void nts::And4081::setName(std::string &name) noexcept
{
    _name = name;
}

void nts::And4081::simulate(std::size_t tick) noexcept
{
    return;
    std::cout << tick << std::endl;
}

nts::Tristate nts::And4081::compute(std::size_t pin) noexcept
{
...
void nts::And4081::setLink(std::size_t pin, nts::IComponent &other, std::size_t otherPin) noexcept
{
    _lks[pin].first = &other;
    _lks[pin].second = otherPin;
}

void nts::And4081::dump() const noexcept
{
    std::cout << " composant 4081and:" << std::endl;
...
void nts::And4081::computeOutput(const std::size_t pin) noexcept
{
    if (3 == pin) {
	computeInput(1);
	computeInput(2);
	_values[pin - 1] = andBlock(1, 2);
    } else if (4 == pin) {
	computeInput(6);
	computeInput(5);
	_values[pin - 1] = andBlock(6, 5);
    } else if (11 == pin) {
	computeInput(12);
	computeInput(13);
	_values[pin - 1] = andBlock(12, 13);
    } else if (10 == pin) {
	computeInput(8);
	computeInput(9);
	_values[pin - 1] = andBlock(8, 9);
    }
}

void nts::And4081::computeInput(const std::size_t pin) noexcept
{
    if (!_lks[pin].first)
	return;
...
nts::Tristate nts::And4081::andBlock(const std::size_t pin1, const std::size_t pin2) noexcept
{
